...
public class NoticeDAO {
	...
	public int insertReview(Connection conn, Review r) {
		...
		try {
			...
			result = pstmt.executeUpdate();
			System.out.println("Reviewresult@"+result);
		} catch (SQLException e) {
			e.printStackTrace();
		}finally {
			close(pstmt);
	...
	public String selectLastSeq_mcode(Connection conn) {
		
		String memberCode = "";
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String query = prop.getProperty("selectLastSeq_mcode");
		try {
			pstmt = conn.prepareStatement(query);
			rset = pstmt.executeQuery();
			if(rset.next()) {
				memberCode = rset.getString("member_code");
			}
			System.out.println("memberCode@dao="+memberCode);
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			close(rset);
	...
	public int updateReview(Connection conn, Review r) {
		...
		try {
			...
			result = pstmt.executeUpdate();
			System.out.println("udpateReview@dao="+result);
	...
	//0212민희 - 조회수 top3 후기글 찾기
	public List<Review> selectTopViewReview(Connection conn) {
		List<Review> rList = new ArrayList<Review>();
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		Review r = null;
		String query = prop.getProperty("selectTopViewReview");
		System.out.println("조회수 탑쓰리 query="+query);
